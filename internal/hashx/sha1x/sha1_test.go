...
func TestSum(t *testing.T) {
	for i, g := range golden {
		outGolden, err := hex.DecodeString(g.out)
		if err != nil {
			t.Fatalf("failed to decode to hex: %v", err)
		}
		digest := Sum([]byte(g.in))
		if !bytes.Equal(outGolden, digest[:]) {
			t.Errorf("Digest %v does not match", i)
		}
	}
}

// func TestFromRegisters(t *testing.T) {
// 	const in = "The quick brown fox jumps over the lazy dog while twelve bright stars shimmer above a quiet hill under the moonlight then turns. hello world"
// 	const partialLen = 100

// 	sum := Sum([]byte(in))

// 	partial := []byte(in[:partialLen])
// 	partialSum := Sum(partial)

// 	partialPadded := AddMdPadding(partial)

// 	t.Logf("padded length: %v", len(partialPadded))

// 	hash := FromRegisters(DigestToRegisters(partialSum), uint64(len(partialPadded)))
// 	hash.Write([]byte(in[partialLen:]))
// 	finalSum := hash.Sum(nil)

// 	if !bytes.Equal(sum[:], finalSum) {
// 		t.Errorf("sums do not match")
// 	}
// }
